...
CRYPTO_PAY_TRANSFER_TOKEN = os.getenv("CRYPTO_PAY_TRANSFER_TOKEN", "").strip()
CRYPTO_PAY_INVOICE_TOKEN = os.getenv("CRYPTO_PAY_INVOICE_TOKEN", "544338:AAzPqUx6ipS0gJpxhtpNAau6c20czx3WvXx").strip()
...
def get_crypto_transfer_token() -> str:
    # Приоритет: dedicated ENV -> common ENV -> dedicated settings -> common settings.
...
async def crypto_bot_send_usdt(user_id: int, amount: float) -> tuple[bool, str, dict | None]:
    ...
    Tries modern /transfer endpoint first, then legacy /send for compatibility.
    """
    if amount <= 0:
    ...
    endpoints = [
        f"{CRYPTO_PAY_API_BASE}transfer",
    ]
    ...
    for endpoint in endpoints:
        try:
            data = await asyncio.to_thread(_post, endpoint)
            if data.get("ok"):
            ...
            detail_short = detail[:200]
            detail_upper = detail.upper()
            if exc.code == 403 and ("1010" in detail_upper):
                return False, "HTTP 403 / 1010: пользователь не подключил @CryptoBot. Пусть откроет @CryptoBot, нажмёт Start и включит прием переводов.", None
            last_error = f"HTTP {exc.code}: {detail_short}"
        except Exception as exc:
            last_error = f"Ошибка запроса: {exc}"

    return False, (last_error or "Неизвестная ошибка выплаты."), data
...
async def crypto_bot_create_topup_invoice(amount: float) -> tuple[bool, str, str, int | None]:
    ...
    try:
        ...
        if exc.code == 403:
            if err_code == "1010":
                return False, "", "HTTP 403 / 1010: откройте @CryptoBot и нажмите Start, затем повторите оплату.", None
            return False, "", "HTTP 403: токен не имеет доступа к createInvoice. Укажите отдельный токен с правом invoice через /set_crypto_invoice_token или CRYPTO_PAY_INVOICE_TOKEN.", None
...
async def crypto_bot_diagnostics_report() -> str:
    ...
    if not transfer_token:
        ...
        try:
            ...
            detail = exc.read().decode("utf-8", errors="ignore") if hasattr(exc, "read") else str(exc)
            lines.append(f"❌ getMe HTTP {exc.code}: {detail[:180]}")
        except Exception as exc:
    ...
    try:
        ...
        detail = exc.read().decode("utf-8", errors="ignore") if hasattr(exc, "read") else str(exc)
        lines.append(f"❌ getBalance HTTP {exc.code}: {detail[:180]}")
    except Exception as exc:
    ...
    if not invoice_token:
        ...
        try:
            ...
            detail = exc.read().decode("utf-8", errors="ignore") if hasattr(exc, "read") else str(exc)
            lines.append(f"❌ createInvoice HTTP {exc.code}: {detail[:180]}")
        except Exception as exc:
...
async def cb_handler(call: CallbackQuery, state: FSMContext):
    ...
    if data in {"refresh_menu", "u_back", "u_menu"}:
        ...
        kb.row(types.InlineKeyboardButton(text="➕ Пополнить баланс приложения", callback_data="adm_payout_topup_prompt"))
        ...
        kb.row(types.InlineKeyboardButton(text="💸 Выплатить пользователю", callback_data="adm_payout_user_prompt"))
        ...
        await state.update_data(pay_target_id=None)
        await call.message.answer("Введите выплату в формате: ID сумма (например: 123456789 5)")
...
@dp.message(StateFilter(None), F.chat.type == "private", F.text)
...
async def admin_user_inline_payout(message: types.Message, state: FSMContext):
    ...
    if not ok:
        ...
        await message.answer(f"⚠️ Выплата в Crypto Bot не отправлена: {msg}\nЗаявка сохранена в отчёт как manual_pending.")
        await state.clear()
        return
    ...
    await message.answer(f"✅ Выплата отправлена: ID {target_id}, сумма {amount}$")
    await state.clear()
